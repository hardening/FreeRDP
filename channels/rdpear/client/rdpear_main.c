                                           ...
                                           NdrContext* rcontext, wStream* s)
{
	...
	if (!ndr_read_ComputeTgsChecksumReq(rcontext, s, &req) || !ndr_treat_deferred_read(rcontext, s))
		return FALSE;
	ComputeTgsChecksumReq_dump(WLog_Get(""), WLOG_DEBUG, &req);

	WINPR_MD_TYPE hashType = WINPR_MD_NONE;
	size_t hashLen = sizeof(checksumBuffer);
	switch (req.ChecksumType) {
	case 0x07: /* CKSUMTYPE_RSA_MD5 */
		hashType = WINPR_MD_MD5;
		hashLen = 16;
		break;
	case 0x0c: /* CKSUMTYPE_HMAC_SHA1_DES3 */
	case 0x0f: /* CKSUMTYPE_HMAC_SHA1_96_AES128 */
	case 0x10: /* CKSUMTYPE_HMAC_SHA1_96_AES256 */
		hashType = WINPR_MD_SHA1;
		hashLen = 20;
		break;
	default:
		WLog_ERR(TAG, "unhandled checksum type 0x%"PRIx32, req.ChecksumType);
		goto out;
	}

	WINPR_DIGEST_CTX *hasher = winpr_Digest_New();
	if (!hasher)
		goto out;

	BOOL tmpRet = winpr_Digest_Init(hasher, hashType) &&
			winpr_Digest_Update(hasher, req.requestBody->Asn1Buffer, req.requestBody->Asn1BufferHints.count) &&
			winpr_Digest_Final(hasher, checksumBuffer, hashLen);
	winpr_Digest_Free(hasher);
	if (!tmpRet)
		goto out;

	SecPkgContext_Asn1Data checksum = {checksumBuffer, (UINT32)hashLen };
	wStream* asn1Payload = rdpear_enc_Checksum(req.ChecksumType, &checksum);
	if (!asn1Payload)
		goto out;
	...
	winpr_HexDump(TAG, WLOG_DEBUG, checksumBuffer, hashLen);

	if (!rdpear_prepare_response(rcontext, RemoteCallKerbComputeTgsChecksum, 0x0000, &wcontext, &retStream) ||
	    !ndr_write_KERB_ASN1_DATA(wcontext, retStream, NULL, &asn1) || /* Checksum */
		!ndr_treat_deferred_write(wcontext, retStream) ||
	    !ndr_end_constructed(wcontext, retStream))
		goto out;
                                               ...
                                               NdrContext* rcontext, wStream* s)
{
	...
	if (!ndr_read_BuildEncryptedAuthDataReq(rcontext, s, &req) || !ndr_treat_deferred_read(rcontext, s))
		return FALSE;
	...
	/* do the encoding */
	wStream* asn1Payload = rdpear_enc_EncryptedData(req.Key->reserved2, &kerbArgs.encrypted);
	if (!asn1Payload)
		goto out;
	...
	if (!rdpear_prepare_response(rcontext, RemoteCallKerbBuildEncryptedAuthData, 0x0000, &wcontext, &retStream) ||
	    !ndr_write_KERB_ASN1_DATA(wcontext, retStream, NULL, &asn1) || /* EncryptedAuthData */
		!ndr_treat_deferred_write(wcontext, retStream) ||
	    !ndr_end_constructed(wcontext, retStream))
		goto out;
...
static void ASN1_KERB_to_SecPkgContext(const KERB_ASN1_DATA *src, SecPkgContext_Asn1Data *dst) {
	dst->len = src->Asn1BufferHints.count;
	dst->data = src->Asn1Buffer;
}
...
static char *KERB_RPC_INTERNAL_NAME_to_charptr(const KERB_RPC_INTERNAL_NAME *src)
{
	if (src->nameHints.count != 1) {
		WLog_ERR(TAG, "implement src->nameHints.count != 1");
		return NULL;
	}

	return KERB_RPC_UNICODESTR_to_charptr(&src->Names[0]);
}

static BOOL rdpear_kerb_CreateApReqAuthenticator(RDPEAR_PLUGIN* rdpear, IWTSVirtualChannelCallback* pChannelCallback,
	   NdrContext* rcontext, wStream* s)
{
	BOOL ret = FALSE;
	NdrContext* wcontext = NULL;
	...
	CreateApReqAuthenticatorResp resp = { 0 };
	SecPkgContext_CreateApReqAuthenticator sspiReq = { 0 };

	WLog_DBG(TAG, "rdpear_kerb_CreateApReqAuthenticator=");
	winpr_HexDump(TAG, WLOG_DEBUG, Stream_Pointer(s), Stream_GetRemainingLength(s));

	if (!ndr_read_CreateApReqAuthenticatorReq(rcontext, s, &req) || !ndr_treat_deferred_read(rcontext, s))
		return FALSE;

	sspiReq.seqNumber = req.SequenceNumber;
	sspiReq.clientName = KERB_RPC_INTERNAL_NAME_to_charptr(req.ClientName);
	if (!sspiReq.clientName)
		goto out;
	sspiReq.realm = KERB_RPC_UNICODESTR_to_charptr(req.ClientRealm);
	if (!sspiReq.realm)
		goto out;
	if (req.AuthData)
		ASN1_KERB_to_SecPkgContext(req.AuthData, &sspiReq.authData);
	if (req.GssChecksum)
		ASN1_KERB_to_SecPkgContext(req.GssChecksum, &sspiReq.checksum);
	sspiReq.keyUsage = req.KeyUsage;

	DWORD status = freerdp_nla_setKerbAttribute(rdpear->rdp_context, SECPKG_CRED_ATTR_CREATE_AP_REQ_AUTH, &sspiReq);
	if (status != SEC_E_OK)
		goto out;

	resp.AuthenticatorTime.QuadPart = sspiReq.authenticatorTime;
	resp.KerbProtocolError = sspiReq.kerbProtocolError;
	resp.Authenticator.Pdu = 6;

	asn1EncodedAuth = rdpear_enc_EncryptedData(0x12/*TODO*/, &sspiReq.authenticator);
	...
	if (!rdpear_prepare_response(rcontext, RemoteCallKerbCreateApReqAuthenticator, 0x0000, &wcontext, &retStream) ||
		!CreateApReqAuthenticatorResp_write(wcontext, retStream, &resp) ||
		!ndr_treat_deferred_write(wcontext, retStream) ||
	    !ndr_end_constructed(wcontext, retStream))
		goto out;
                                               ...
                                               NdrContext* rcontext, wStream* s)
{
	...
	UnpackKdcReplyBodyResp resp = { 0 };
	SecPkgContext_UnpackKdcReplyBody sspiReq = { 0 };

	if (!ndr_read_UnpackKdcReplyBodyReq(rcontext, s, &req) || !ndr_treat_deferred_read(rcontext, s))
		return FALSE;

	ASN1_KERB_to_SecPkgContext(req.EncryptedData, &sspiReq.encryptedData);
	sspiReq.keyUsage = req.KeyUsage;

	DWORD status = freerdp_nla_setKerbAttribute(rdpear->rdp_context, SECPKG_CRED_ATTR_UNPACK_REPLY_BODY, &sspiReq);
	if (status != SEC_E_OK)
		goto out;

	resp.KerbProtocolError = sspiReq.KerbProtocolError;
	resp.ReplyBody.Pdu = 70;
	resp.ReplyBody.Asn1Buffer = sspiReq.replyBody.data;
	resp.ReplyBody.Asn1BufferHints.count = sspiReq.replyBody.len;

	if (!rdpear_prepare_response(rcontext, RemoteCallKerbUnpackKdcReplyBody, 0x0000, &wcontext, &retStream) ||
		!UnpackKdcReplyBodyResp_write(wcontext, retStream, &resp) ||
		!ndr_treat_deferred_write(wcontext, retStream) ||
	    !ndr_end_constructed(wcontext, retStream))
		goto out;
                                  ...
                                  RdpEarPackageType packageType, wStream* s)
{
	...
	switch (callId)
	{
		...
		case RemoteCallKerbCreateApReqAuthenticator:
			if (rdpear_kerb_CreateApReqAuthenticator(rdpear, pChannelCallback, context, &commandStream))
				ret = CHANNEL_RC_OK;
			break;
		...
		case RemoteCallKerbUnpackKdcReplyBody:
			if (rdpear_kerb_UnpackKdcReplyBody(rdpear, pChannelCallback, context,
			                                       &commandStream))
				ret = CHANNEL_RC_OK;
			break;
