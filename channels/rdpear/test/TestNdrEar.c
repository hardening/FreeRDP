...
int TestNdrEarRead(int argc, char* argv[])
{
	...
	BYTE *payload4 = parseHexBlock(
	   "03 01 03 01 \
		...
		97 4c f2 5c 0b 49 85 72 92 94 54 00", &sizeofPayload4);
	...
	if (!ndr_skip_bytes(context, s, 4) || /* skip union id */
		!ndr_read_CreateApReqAuthenticatorReq(context, s, &createApReqAuthenticatorReq) ||
		!ndr_treat_deferred_read(context, s) ||
		createApReqAuthenticatorReq.KeyUsage != 7 ||
		...
		!createApReqAuthenticatorReq.AuthData || createApReqAuthenticatorReq.AuthData->Asn1BufferHints.count != 2 ||
		!createApReqAuthenticatorReq.SkewTime || createApReqAuthenticatorReq.SkewTime->QuadPart != 0)
		goto out;
	CreateApReqAuthenticatorReq_destroy(context, &createApReqAuthenticatorReq);
	ndr_context_reset(context);

	/* ============ sucessfull end of test =============== */
	retCode = 0;
out:
	ndr_context_destroy(&context);
	return retCode;
