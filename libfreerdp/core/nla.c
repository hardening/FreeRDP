...
{
	if (!Stream_EnsureRemainingCapacity(s, (4 * 4) + 16 + ticket->ServiceTicketLength +
	                                           ticket->TicketGrantingTicketLength))
		return FALSE;
...
BOOL nla_get_KERB_TICKET_LOGON(rdpNla* nla, KERB_TICKET_LOGON* logonTicket)
{
	...
	return table->QueryContextAttributesA(&context, SECPKG_CRED_ATTR_TICKET_LOGON, logonTicket) ==
	       SEC_E_OK;
}
