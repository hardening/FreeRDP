...
{
	/* "krbtgt/" + realm + "@" + realm */
	int len = 7 + realm->length + 1 + realm->length + 1;
                                                  ...
                                                  KERB_TICKET_LOGON* ticketLogon)
{
	...
	if (krb_log_exec(krb5_sname_to_principal, credentials->ctx, context->targetHost, "HOST",
...
static SECURITY_STATUS kerberos_EncryptedLen(KRB_CONTEXT* context, KRB_CREDENTIALS* credentials,
                                             SecPkgContext_KerbEncryptedLen* encryptedLen)
{
	...
	size_t len;
	krb5_error_code rv = krb_log_exec(krb5_c_encrypt_length, credentials->ctx,
	                                  encryptedLen->encType, encryptedLen->plainLen, &len);
	encryptedLen->encodedLen = len;
	return krb5_error_to_SECURITY_STATUS(rv);
}

#if 0
static void key_to_krb5_keyblock(const SecPkgContext_Key* key, krb5_keyblock* keyblock)
{
	keyblock->enctype = key->encType;
	keyblock->length = key->keyLen;
	keyblock->contents = key->key;
}
#endif

static void asn1data_to_krb5_data(const SecPkgContext_Asn1Data* asn1, krb5_data* data)
{
	data->data = (char*)asn1->data;
	data->length = asn1->len;
}

static SECURITY_STATUS kerberos_ComputeTgsChecksum(KRB_CONTEXT* context,
                                                   KRB_CREDENTIALS* credentials,
                                                   SecPkgContext_ComputeTgsChecksum* checksum)
{
	if (!checksum || !checksum->requestBody.data || !checksum->checksum.data)
		return SEC_E_INVALID_PARAMETER;

	krb5_keyblock keyblock;
	krb5_data requestBody;
	asn1data_to_krb5_data(&checksum->requestBody, &requestBody);

	krb5_checksum checksumRes;
	krb5_error_code rv = krb_log_exec(krb5_c_make_checksum, credentials->ctx, 0, &keyblock,
	                                  KRB5_KEYUSAGE_TGS_REQ_AUTH_CKSUM, &requestBody, &checksumRes);

	if (rv)
		return krb5_error_to_SECURITY_STATUS(rv);

	if (checksumRes.length > checksum->checksum.len)
		return SEC_E_BUFFER_TOO_SMALL;

	memcpy(checksum->checksum.data, checksumRes.contents, checksumRes.length);
	checksum->checksum.len = checksumRes.length;
	return SEC_E_OK;
}

krb5_error_code getCredsTgt(KRB_CREDENTIALS* credentials, krb5_creds **pret)
{
	char *tgtName = NULL;
	krb5_error_code rv;
	krb5_creds matchCred = { 0 };

	if ((rv = krb_log_exec(krb5_cc_get_principal, credentials->ctx, credentials->ccache, &matchCred.client)))
		return rv;

	krb5_data *realm = &matchCred.client->realm;

	/* build the KRBTGT from the realm of the ccache
	 *            "krbtgt/" <REALM>      "@"   <REALM>      \x00 */
	size_t allocLen = 7 + realm->length + 1 + realm->length + 1;
	tgtName = (char*)malloc(allocLen);
	if (!tgtName) {
		rv = KRB5_CC_NOMEM;
		goto out;
	}

	char *target = tgtName + 7;
	strcpy(tgtName, "krbtgt/");
	memcpy(target, realm->data, realm->length);
	target += realm->length;
	*target = '@';
	target++;
	memcpy(target, realm->data, realm->length);
	...
	*target = '\x00';
	...
	rv = krb_log_exec(krb5_parse_name, credentials->ctx, tgtName, &matchCred.server);
	if (rv)
		return rv;

	rv = krb_log_exec(krb5_get_credentials, credentials->ctx, KRB5_GC_CACHED, credentials->ccache, &matchCred, pret);
out:
	krb5_free_cred_contents(credentials->ctx, &matchCred);
	free(tgtName);
	return rv;
...
kerberos_BuildEncryptedAuthData(KRB_CONTEXT* context, KRB_CREDENTIALS* credentials,
                                SecPkgContext_KerbBuildEncryptedAuthData* buildEncrypted)
{
	if (!buildEncrypted ||
	    !buildEncrypted->plain.data || !buildEncrypted->plain.len ||
	    !buildEncrypted->encrypted.data)
		return SEC_E_INVALID_PARAMETER;

	int encType = 0x12;
	size_t elen;
	krb5_error_code rv =
	    krb_log_exec(krb5_c_encrypt_length, credentials->ctx, encType /*buildEncrypted->key.encType*/,
	                 buildEncrypted->plain.len, &elen);

	if (rv)
		return krb5_error_to_SECURITY_STATUS(rv);

	if (elen > buildEncrypted->encrypted.len)
		return SEC_E_BUFFER_TOO_SMALL;

	krb5_creds *tgt;
	if ((rv = getCredsTgt(credentials, &tgt)))
		return krb5_error_to_SECURITY_STATUS(rv);

	krb5_keyblock *subkey = &tgt->keyblock;/*NULL;
	rv = krb_log_exec(krb5_generate_subkey, credentials->ctx, &tgt->keyblock, &subkey);
	if (rv)
		return krb5_error_to_SECURITY_STATUS(rv);*/

	krb5_data plain;
	plain.data = (char*)buildEncrypted->plain.data;
	plain.length = buildEncrypted->plain.len;

	krb5_enc_data cipher;
	cipher.enctype = encType/*buildEncrypted->key.encType*/;
	cipher.kvno = 5;
	cipher.ciphertext.data = (char*)buildEncrypted->encrypted.data;
	cipher.ciphertext.length = buildEncrypted->encrypted.len;

	rv = krb_log_exec(krb5_c_encrypt, credentials->ctx, subkey,
	                  (krb5_keyusage)buildEncrypted->keyUsage, NULL, &plain, &cipher);
	return krb5_error_to_SECURITY_STATUS(rv);
}


static BOOL checksum_to_krb5_checksum(const SecPkgContext_Asn1Data *checksum, krb5_checksum *k5sum)
{
	WinPrAsn1Decoder decoder = { 0 };
	WinPrAsn1Decoder seq = { 0 };
	WinPrAsn1Decoder_InitMem(&decoder, WINPR_ASN1_DER, checksum->data, checksum->len);

	WinPrAsn1_INTEGER id;
	WinPrAsn1_OctetString os;
	BOOL error = FALSE;
	if (!WinPrAsn1DecReadSequence(&decoder, &seq) ||
		!WinPrAsn1DecReadContextualInteger(&seq, 0, &error, &id) || id != CKSUMTYPE_RSA_MD5 ||
		!WinPrAsn1DecReadContextualOctetString(&seq, 1, &error, &os, FALSE))
		return FALSE;

	k5sum->checksum_type = id;
	k5sum->length = os.len;
	k5sum->contents = os.data;
	return TRUE;
}

#if 0
static SECURITY_STATUS buildName(KRB_CREDENTIALS* credentials, const char *clientName, const char *realm, krb5_principal *pprincipal) {
	int rv;
	rv = krb_log_exec(krb5_parse_name_flags, credentials->ctx, clientName, KRB5_PRINCIPAL_PARSE_NO_DEF_REALM, pprincipal);
	if (rv)
		 return krb5_error_to_SECURITY_STATUS(rv);

	rv = krb_log_exec(krb5_set_principal_realm, credentials->ctx, *pprincipal, realm);
	return krb5_error_to_SECURITY_STATUS(rv);
}
#endif

static krb5_error_code krb5_encrypt_helper(krb5_context context,
...
UINT64 krb5_time_to_FILETIME(const krb5_timestamp *ts, krb5_int32 usec) {
	return ((*ts + FILETIME_TO_UNIX_OFFSET_S) * (1000 * 1000) + usec) * 10;
}

static BOOL extractAuthData(const SecPkgContext_Asn1Data *authData, krb5_authdata *target, BOOL *pisNull)
{
	WinPrAsn1Decoder decoder, dec;

	WinPrAsn1Decoder_InitMem(&decoder, WINPR_ASN1_DER, authData->data, authData->len);
	if (!WinPrAsn1DecReadSequence(&decoder, &dec))
		return FALSE;

	wStream rest = WinPrAsn1DecGetStream(&dec);
	*pisNull = (Stream_GetRemainingLength(&rest) == 0);
	target->contents = Stream_Pointer(&rest);
	target->length = Stream_GetRemainingLength(&rest);

	return TRUE;
}

static SECURITY_STATUS
kerberos_CreateApReqAuthenticator(KRB_CONTEXT* context, KRB_CREDENTIALS* credentials, SecPkgContext_CreateApReqAuthenticator* req)
{
	if (!req)
		return SEC_E_INVALID_PARAMETER;
	...
	//krb5_keyblock keyblock;
	//krb5_keyblock subkey;
	krb5_keyblock *subkeyPtr = NULL;
	krb5_enc_data authent_enc = { 0 };

	krb5_checksum checksum;
	if (!checksum_to_krb5_checksum(&req->checksum, &checksum))
		goto out;

	krb5_creds* tgtCred = NULL;
	if ((rv = getCredsTgt(credentials, &tgtCred)))
		return krb5_error_to_SECURITY_STATUS(rv);

	krb5_authenticator authent = { 0 };
	krb5_authdata authData = { 0 };

	authData.ad_type = req->keyUsage;

	BOOL isEmpty = FALSE;
	if (!extractAuthData(&req->authData, &authData, &isEmpty)) {
		rv = ASN1_OVERFLOW;
		goto out;
	}

	krb5_authdata *authDataPtr = isEmpty ? NULL : &authData;

	authent.subkey = subkeyPtr;
	authent.seq_number = req->seqNumber;
	authent.checksum = &checksum;
	authent.client = tgtCred->client;
	if (req->authData.len != 0)
		WLog_ERR(TAG, "should handle AuthData");
	authent.authorization_data = &authDataPtr /*tgtCred->authdata*/;

	rv = krb5_us_timeofday(credentials->ctx, &authent.ctime, &authent.cusec);
	if (rv)
		goto out;

	rv = encode_krb5_authenticator(&authent, &authent_asn1);
	if (rv)
		goto out;
	rv = krb_log_exec(krb5_encrypt_helper, credentials->ctx, &tgtCred->keyblock,
	                              req->keyUsage /*KRB5_KEYUSAGE_TGS_REQ_AUTH*/, authent_asn1,
	                              &authent_enc);
	if (rv)
		goto out;

	req->kerbProtocolError = rv;
	req->authenticatorTime = krb5_time_to_FILETIME(&authent.ctime, authent.cusec);
	req->authenticator.len = authent_enc.ciphertext.length;
	req->authenticator.data = (BYTE*)authent_enc.ciphertext.data;
out:
	return krb5_error_to_SECURITY_STATUS(rv);
}

static SECURITY_STATUS
kerberos_UnpackReplyBody(KRB_CONTEXT* context, KRB_CREDENTIALS* credentials, SecPkgContext_UnpackKdcReplyBody* unpackReply)
{
	krb5_error_code rv;

	if (!unpackReply || !unpackReply->replyBody.data || !unpackReply->replyBody.len ||
	    !unpackReply->encryptedData.data || unpackReply->encryptedData.len)
		return SEC_E_INVALID_PARAMETER;
	...
	krb5_enc_data ciphered;
	int encType = 0x12;
	ciphered.enctype = encType; /* TODO key->encType ?*/
	ciphered.kvno = 5;
	ciphered.ciphertext.data = (char *)unpackReply->encryptedData.data;
	ciphered.ciphertext.length = unpackReply->encryptedData.len;

	krb5_data plain;
	plain.data = (char *)unpackReply->replyBody.data;
	plain.length = unpackReply->replyBody.len;

	rv = krb_log_exec(krb5_c_decrypt, credentials->ctx, &tgt->keyblock, unpackReply->keyUsage, NULL, &ciphered, &plain);
	unpackReply->KerbProtocolError = rv;
	unpackReply->replyBody.len = plain.length;
	return krb5_error_to_SECURITY_STATUS(rv);
}
                                                                  ...
                                                                  ULONG ulAttribute, void* pBuffer)
{
	...
	switch (ulAttribute)
	{
		...
		case SECPKG_CRED_ATTR_ENCRYPTED_LEN:
			return kerberos_EncryptedLen(context, credentials, (SecPkgContext_KerbEncryptedLen*)pBuffer);

		case SECPKG_CRED_ATTR_BUILD_ENCRYPTED_AUTH_DATA:
			return kerberos_BuildEncryptedAuthData(context, credentials, (SecPkgContext_KerbBuildEncryptedAuthData*)pBuffer);

		case SECPKG_CRED_ATTR_CREATE_AP_REQ_AUTH:
			return kerberos_CreateApReqAuthenticator(context, credentials, (SecPkgContext_CreateApReqAuthenticator*)pBuffer);

		case SECPKG_CRED_ATTR_COMPUTE_TGS_CHECKSUM:
			return kerberos_ComputeTgsChecksum(context, credentials, (SecPkgContext_ComputeTgsChecksum*)pBuffer);

		case SECPKG_CRED_ATTR_UNPACK_REPLY_BODY:
			return kerberos_UnpackReplyBody(context, credentials, (SecPkgContext_UnpackKdcReplyBody*)pBuffer);

		default:
			WLog_ERR(TAG, "TODO: QueryContextAttributes implement ulAttribute=0x%08" PRIx32, ulAttribute);
			return SEC_E_UNSUPPORTED_FUNCTION;
	}
#else
	return SEC_E_UNSUPPORTED_FUNCTION;
