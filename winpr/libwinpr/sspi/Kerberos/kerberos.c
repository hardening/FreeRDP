                                                                  ...
                                                                  ULONG ulAttribute, void* pBuffer)
{
	...
	switch (ulAttribute)
	{
		case SECPKG_ATTR_SIZES:
		{
			...
			{
				krb5_error_code rv = krb_log_exec(krb5glue_crypto_length, creds->ctx, key,
				                                  KRB5_CRYPTO_TYPE_HEADER, &header);
				if (rv)
					return rv;
				rv = krb_log_exec(krb5glue_crypto_length, creds->ctx, key, KRB5_CRYPTO_TYPE_PADDING,
				                  &pad);
				if (rv)
					return rv;
				rv = krb_log_exec(krb5glue_crypto_length, creds->ctx, key, KRB5_CRYPTO_TYPE_TRAILER,
				                  &trailer);
				if (rv)
					return rv;
			...
			{
				krb5_error_code rv =
				    krb_log_exec(krb5glue_crypto_length, creds->ctx, key, KRB5_CRYPTO_TYPE_CHECKSUM,
		...
		case SECPKG_CRED_ATTR_TICKET_LOGON:
		{
			...
			KERB_TICKET_LOGON* ticketLogon = (KERB_TICKET_LOGON*)pBuffer;

			return kerberos_ATTR_TICKET_LOGON(context, credentials, ticketLogon);
		}
		default:
